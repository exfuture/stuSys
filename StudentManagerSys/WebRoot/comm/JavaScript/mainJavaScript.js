...
function openDeleteDialog(url,confirmString){
	...
	if(confirm(c)){
		return window.showModalDialog(url,"window123","dialogHeight:300px;dialogWidth:500px;resizable:no;help:yes;status:no;scroll:no");
	}
	return false;
}
...
return window.showModalDialog(url,title,"dialogHeight:300px;dialogWidth:500px;resizable:no;help:yes;status:no;scroll:no");	
}
function openDialog(url,title){
	if(openModalDialog(url,title)){
		window.location.reload(true);
	}
}

function openModalDialogSeal(url,title){
return window.showModalDialog(url,title,"dialogHeight:300px;dialogWidth:500px;resizable:no;help:yes;status:no;scroll:no");	
}
function openDialogSeal(url,title){
	if(openModalDialog(url,title)){
		window.location.reload(true);
	}
}
...
{    
 ...
 if   ((event.ctrlKey)&&(event.keyCode==86))       //屏蔽   Ctrl+V   
     ...
     if(txt.length>maxlen-1){
     	alert("书写内容不能超过"+maxlen+"个字！");
     obj.value=txt.substring(0,maxlen-1);
     }    
             
}

//定义窗口改变函数
function changeplace(row,col)
{
	window.moveTo(row,col);
}
//定义窗口大小改变函数
function changesize(xsize,ysize)
{
